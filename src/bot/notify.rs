...
mod tests {
    ...
    fn polling_event(event_type: &str, actor: &str, payload: serde_json::Value) -> RepoEvent {
        RepoEvent {
            ...
            actor: EventActor { login: actor.to_string() },
            payload,
    ...
    fn html_special_chars_are_escaped() {
        ...
        let msg = format_webhook_event("issues", &data, "o", "r").unwrap();
        assert!(!msg.contains("<script>"), "raw script tag should not appear");
    ...
    fn polling_issue_opened() {
        let ev = polling_event("IssuesEvent", "alice", serde_json::json!({
            "action": "opened",
    ...
    fn polling_skips_events_from_current_user() {
        let ev = polling_event("IssuesEvent", "myself", serde_json::json!({
            "action": "opened",
    ...
    fn polling_case_insensitive_user_skip() {
        let ev = polling_event("IssuesEvent", "MyUser", serde_json::json!({
            "action": "opened",
    ...
    fn polling_pr_merged() {
        let ev = polling_event("PullRequestEvent", "dev", serde_json::json!({
            "action": "closed",
            "pull_request": {
                "number": 8,
                "title": "Merge me",
    ...
    fn polling_push_event() {
        let ev = polling_event("PushEvent", "dev", serde_json::json!({
            "ref": "refs/heads/main",
            "commits": [
